...
export default class LocalEchoController {
    ...
    * additions to the input.
    ...
    applyPromptOffset(input, offset) {
        const newInput = this.applyPrompts([...input].slice(0, offset).join(""));
    ...
    clearInput(cursor) {
        ...
        const promptCursor = this.applyPromptOffset(this._input, cursor);
     ...
     */
    setInput(newInput, clearInput = true, oldCursor = null) {
        ...
        // Clear current input
        if (clearInput) this.clearInput(oldCursor);
        ...
        if (this._cursor > newInputLength) {
            this._cursor = newInputLength;
        }
    ...
    /*
     * Try to move the cursor up in a multi-line, returning false if the 
    ...
    handleCursorUp() {
        ...
        newCursor += Math.min([...lines[upRow]].length - 1, col);
    ...
    handleCursorDown() {
        ...
        const downRow = row + 1;
        let newCursor = 0;
        for (let i = 0; i < downRow; i++) {
            newCursor += [...lines[i]].length;
        }

        newCursor += Math.min([...lines[downRow]].length - 1, col);
        this.setCursor(newCursor);
        return true;
    }
    ...
    handleCursorErase(backspace) {
        ...
        if (backspace) {
            if (_cursor <= 0) return;
            const newInput = [..._input].slice(0, _cursor - 1).join("") + [..._input].slice(_cursor).join("");
            this.clearInput(this._cursor);
            this._cursor -= 1;
            this.setInput(newInput, false);
        } else {
            const newInput = [..._input].slice(0, _cursor).join("") + [..._input].slice(_cursor + 1).join("");
    ...
    handleCursorInsert(data) {
        const { _cursor, _input } = this;
        const newInput = [..._input].slice(0, _cursor).join("") + data + [..._input].slice(_cursor).join("");
        this._cursor += [...data].length;
        this.setInput(newInput, true, _cursor);
    }
    ...
    handleReadComplete() {
        ...
        // Move the cursor to the end of the current input if it's not
        // already
        let inputSize = [...this._input].length;
        if (this._cursor < inputSize) {
            this.handleCursorMove(inputSize - this._cursor);
        }
