...
export class Tty {
    ...
    public computeLayout(promptSize: Position, line: LineBuffer): Layout {
        ...
        const end = (pos == line.buf.length) ? { ...cursor } :
    ...
    public refreshLine(prompt: string, line: LineBuffer, oldLayout: Layout, newLayout: Layout) {
        ...
        if (end_pos.col == 0 && end_pos.row > 0 && line.buf[line.buf.length - 1] != '\n') {
            this.write("\n");
        }
